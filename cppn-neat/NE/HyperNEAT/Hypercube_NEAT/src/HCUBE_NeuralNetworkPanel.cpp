...
namespace HCUBE
{
    ...
    {
        {
            ...
            cout_ << "CREATING OUTPUT BITMAP OF SIZE " << dc.GetSize().x << ',' << dc.GetSize().y << endl;

            cout_ << "PANEL SIZE?: " << GetSize().x << ',' << GetSize().y << '\n';
            ...
            if (individual)
            {
                ...
                for (int a=0;a<individual->getNodesCount();a++)
                {
                    ...
                    //cout_ << yCoord << '/' << node->getDrawingPosition() << endl;
                ...
                //cout_ << individual->getLinksCount() << endl;
    ...
    void NeuralNetworkPanel::paint(wxPaintEvent& event)
    {
        ...
        {
            cout_ << __FILE__ << ": Bitmap is not ok to draw\n";
        }
    }
