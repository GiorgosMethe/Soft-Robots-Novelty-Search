...
Globals::Globals()
    :
      ...
      speciesCounter(0)
{
    cout_ << "Populating sigmoid table...";
    ...
    cout_ << "Loading Parameter data from defaults" << endl;
    ...
    parameters.insert("ExperimentType", 1.0);
      ...
      speciesCounter(0)
{
    ...
    cout_ << "Loading Parameter data from " << fileName << endl;
...
void Globals::initRandom()
{
    ...
    if (randomSeed<0.0)
    {
        cout_ << "Seeding random generator to time\n";
        random = Random(); //use time as the seed
    }
    else
    {
        cout_ << "Seeing random generator with given seed: " << uint(randomSeed) << endl;
...
{
    cout_ << "Reseeding random with seed: " << newSeed << endl;
