...
public:
    ...
    inline shared_ptr<GeneticIndividual> getIndividualAt(int i)
    {
        return currentIndividuals.at(i);
    }
...
private:
    std::pair<int, int> chooseParentsViaCompetition(int lastIndex);

    int chooseParentViaCompetition(int lastIndex);

    std::pair<int, int> chooseParentsRandomly(int lastIndex);

    int Competition, CompetitionSize;
};
