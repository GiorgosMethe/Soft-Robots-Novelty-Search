...
public:
    ...
    std::vector<int> VoxBotSimFile;
    float MaterialProperties[4][4];
    ...
    inline void setFitnessBackUp(double _fitnessBackUp)
    {
        fitnessBackUp = _fitnessBackUp;
    }

    inline double getFitnessBackUp()
    {
    ...
    inline void isNovel(bool _novel)
    {
        novel = _novel;
    }

    /**
      * getSignatureDifference: returns the difference between this individual's and another's behavior signature
      */
    NEAT_DLL_EXPORT double getSignatureDifference(shared_ptr<GeneticIndividual> other);
