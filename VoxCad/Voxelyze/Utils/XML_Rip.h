...
#if USE_STRINGSTREAM
    bool FindLoadElement(std::string tag, double* pDouble, bool StayDown = false){if (FindLoadElement(tag, &tmp, StayDown)){ std::stringstream ss(tmp); ss >> *pDouble; return true;}return false;}
#else
