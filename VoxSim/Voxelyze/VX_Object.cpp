...
/*! return a string with the current palette
@param[out] a std::string with each element of the palette
*/
std::string CVX_Object::printPaletteElements()
{
    if(Palette.size() == 0) return NULL;
    std::string paletteStr = "";
    for (unsigned int i = 0; i < Palette.size(); ++i) {
        CVXC_Material tmp = Palette.at(i);
        paletteStr += "palette element | ";
        paletteStr += tmp.GetName() + "\n";
    }
    return paletteStr;
}
...
bool CVXC_Structure::ReadXML(CXML_Rip* pXML, std::string Version, std::string* RetMessage)
{
    ...
    if (Version == "" || Version == "0.93" || Version == "0.94"){ //newest version
        ...
        for (int i=0; i<Z_Voxels; i++){
            ...
            if ((int) DataIn.length() != X_Voxels*Y_Voxels){
...
// returns the number of connections between the voxels in a structure
int CVXC_Structure::GetConnectionsNum()
{
    int num = 0;
    for (int x = 0; x < X_Voxels; ++x)
    {
        for (int y = 0; y < Y_Voxels; ++y)
        {
            for (int z = 0; z < Z_Voxels; ++z)
            {
                ...
                if(pData[index] != 0)
                {
                    if(x > 0) num += (pData[GetIndex(x-1, y, z)] != 0);
                    if(y > 0) num += (pData[GetIndex(x, y-1, z)] != 0);
                    if(z > 0) num += (pData[GetIndex(x, y, z-1)] != 0);

                    if(x < X_Voxels) num += (pData[GetIndex(x+1, y, z)] != 0);
                    if(y < Y_Voxels) num += (pData[GetIndex(x, y+1, z)] != 0);
                    if(z < Z_Voxels) num += (pData[GetIndex(x, y, z+1)] != 0);
                }
            }
        }
    }
