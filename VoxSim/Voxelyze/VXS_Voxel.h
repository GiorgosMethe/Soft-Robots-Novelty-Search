...
public:
    ...
    inline vfloat GetCurScale() {return Scale;}
    const inline Vec3D<> GetCurVel() const {return Vel;}
    const inline Vec3D<> GetCurAngVel() const {return AngVel;}
    inline vfloat GetPressure() const {return Pressure;}
    inline vfloat GetCurKineticE() const {return KineticEnergy;}
    inline bool GetCurStaticFric() const {return StaticFricFlag;}
    inline vfloat GetCurAbsDisp() const {return (Vec3D<>(Pos)-GetNominalPosition()).Length();}
