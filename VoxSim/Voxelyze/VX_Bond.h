...
public:
    ...
    template <typename T> void ToXDirBond(Vec3D<T>* const pVec) const {switch (ThisBondAxis){case AXIS_Y: {T tmp = pVec->x; pVec->x=pVec->y; pVec->y = -tmp; break;} case AXIS_Z: {T tmp = pVec->x; pVec->x=pVec->z; pVec->z = -tmp; break;}}}
