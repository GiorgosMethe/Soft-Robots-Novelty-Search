...
public:
    ...
    vfloat NormalizeOnAxis(Vec3D Axis) {T l = sqrt(Axis.getX() * x*x + Axis.getY() *y*y + Axis.getZ() * z*z); if (l > 0) {if(Axis.getX())x /= l; else x = (T)0; if(Axis.getY())y /= l; else y = (T)0; if(Axis.getZ())z /= l; else z = (T)0;} return l;} //normalizes this vector on selected axis set others to zero
