...
bool CVX_Sim::TimeStep(std::string* pRetMessage)
{
    ...
    if (StopConditionMet()){
        if (pRetMessage) *pRetMessage = "Simulation Ended. Stop condition met.\n";
        return false;
    }
...
bool CVX_Sim::UpdateStats(std::string* pRetMessage) //updates simulation state (SS)
{
    ...
    SS.LastCM = SS.CurCM;
    if (true) SS.CurCM = GetCM(); //calculate center of mass
    SS.TotalObjTrajectoryLength += (SS.LastCM - SS.CurCM).Length() * 1000; // Keep whole trajectory length
    if (CurStepCount == 1) SS.LastCM = SS.CurCM; // last equals current for the first step
